...
class ChatifyApp(ctk.CTk):
    def __init__(self, user_data):
        ...
        # --- LAYOUT CONFIGURATION for a narrower, responsive sidebar ---
        # Final, more aggressive adjustment for a noticeably narrower sidebar
        self.grid_columnconfigure(0, weight=1, minsize=260) # Sidebar column with a smaller minsize
        self.grid_columnconfigure(1, weight=5)              # Chat area column with a much higher weight
    ...
    def display_messages(self, messages):
        for msg in messages:
            ...
            message_container.grid(sticky=anchor, padx=(0, 100) if not is_sent else (100, 0), pady=2)

            bubble = ctk.CTkFrame(message_container, fg_color=bubble_color, corner_radius=15); bubble.pack(anchor=anchor, pady=2)
            
            msg_label = ctk.CTkLabel(bubble, text=msg["text"], justify="left", font=ctk.CTkFont(size=13), text_color=TEXT_COLOR)
    ...
    def create_sidebar(self):
        ...
        profile_frame.grid(row=0, column=0, sticky="ew", padx=15, pady=20)
        profile_frame.grid_columnconfigure(1, weight=1)
        
        user_avatar = self.load_image_from_url(self.user_data.get("avatar_url"), size=(40, 40)) # Resized avatar
        avatar_label = ctk.CTkLabel(profile_frame, text="", image=user_avatar); avatar_label.grid(row=0, column=0, sticky="w")
        username_label = ctk.CTkLabel(profile_frame, text=self.user_data.get("username", "User"), font=ctk.CTkFont(size=16, weight="bold"), text_color=TEXT_COLOR, anchor="w") # Resized font
        username_label.grid(row=0, column=1, sticky="ew", padx=10) # Reduced padding
        
        logout_icon_path = "logout.png"
        if os.path.exists(logout_icon_path):
            logout_image = ctk.CTkImage(Image.open(logout_icon_path).resize((20, 20), Image.Resampling.LANCZOS)) # Resized icon
            logout_button = ctk.CTkButton(profile_frame, text="", image=logout_image, width=28, height=28,
        ...
        search_frame.grid(row=1, column=0, sticky="ew", padx=15, pady=(0, 15))
        search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search...", height=34, corner_radius=10, fg_color=CHAT_AREA_COLOR, border_width=1, border_color=BORDER_COLOR) # Resized search
        ...
        contacts_frame.grid(row=2, column=0, sticky="nswe", padx=5, pady=0)
        
        for i in range(10):
            contact_data = {"name": f"Contact {i+1}", "avatar_url": None, "id": i}
            contact_item_frame = ctk.CTkFrame(contacts_frame, fg_color="transparent", height=55, corner_radius=10, cursor="hand2") # Reduced height
            contact_item_frame.pack(fill="x", pady=1, padx=5)
            ...
            contact_avatar = self.load_image_from_url(None, size=(38,38)) # Resized avatar
            avatar = ctk.CTkLabel(contact_item_frame, text="", image=contact_avatar); avatar.grid(row=0, column=0, rowspan=2, padx=(5, 10), pady=10)
            username = ctk.CTkLabel(contact_item_frame, text=contact_data["name"], font=ctk.CTkFont(size=12, weight="bold"), text_color=TEXT_COLOR, anchor="w"); username.grid(row=0, column=1, sticky="sew", pady=(10, 0)) # Adjusted padding
            message = ctk.CTkLabel(contact_item_frame, text="Last message...", font=ctk.CTkFont(size=10), text_color=SECONDARY_TEXT_COLOR, anchor="w"); message.grid(row=1, column=1, sticky="new", pady=(0, 10)) # Adjusted padding
    ...
    def create_chat_area(self):
        ...
        top_bar = ctk.CTkFrame(self.main_chat_frame, fg_color=SIDEBAR_COLOR, corner_radius=0, height=70, border_width=1, border_color=BORDER_COLOR); top_bar.grid(row=0, column=0, sticky="ew")
        self.contact_avatar_label = ctk.CTkLabel(top_bar, text=""); self.contact_avatar_label.pack(side="left", padx=20)
        self.contact_name_label = ctk.CTkLabel(top_bar, text="", font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT_COLOR); self.contact_name_label.pack(side="left")
        
        self.messages_area = ctk.CTkScrollableFrame(self.main_chat_frame, fg_color="transparent"); self.messages_area.grid(row=1, column=0, sticky="nswe", padx=15, pady=10); self.messages_area.grid_columnconfigure(0, weight=1)
        self.messages_area.bind("<Configure>", self._on_chat_area_resize)

        input_bar = ctk.CTkFrame(self.main_chat_frame, fg_color=SIDEBAR_COLOR, corner_radius=0, height=80, border_width=1, border_color=BORDER_COLOR); input_bar.grid(row=2, column=0, sticky="ew")
        message_entry = ctk.CTkEntry(input_bar, placeholder_text="Type a message...", height=40, fg_color=CHAT_AREA_COLOR, border_width=1, border_color=BORDER_COLOR, corner_radius=12, font=ctk.CTkFont(size=13)); message_entry.pack(side="left", expand=True, fill="x", padx=15, pady=15)
        send_button = ctk.CTkButton(input_bar, text="➤", font=ctk.CTkFont(size=22), width=40, height=40, corner_radius=20, fg_color=PRIMARY_GREEN, hover_color="#06A542"); send_button.pack(side="right", padx=(0,15), pady=15)

if __name__ == "__main__":
    user_info = {"user_id": "test_id", "username": "Test User", "avatar_url": None} # Shortened username for better fit
    if len(sys.argv) > 1:
        try: 
